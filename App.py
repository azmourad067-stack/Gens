...
def prepare_features_from_partants(dfp, stats_df=None):
    ...
    # normalize columns that might appear in different languages
    # ensure expected columns exist
    for c in ['Cote','Poids','Musique','Numéro de corde','Num','Age','Âge/Sexe']:
        if c not in df.columns:
            # maybe similar names exist; do nothing else
            pass
    ...
    if 'Musique' in df.columns:
        ...
        # fill defaults
        df['wins'] = 0; df['places'] = 0; df['total_races'] = 0
        df['win_rate'] = 0.0; df['place_rate'] = 0.0; df['recent_form'] = 0.0; df['avg_pos'] = np.nan
    ...
    # chosen features
    # keep a consistent feature order
    features = {
        'odds_inv': df['odds_inv'],
        'win_rate': df.get('win_rate', pd.Series(0.0, index=df.index)),
        'place_rate': df.get('place_rate', pd.Series(0.0, index=df.index)),
        'recent_form': df.get('recent_form', pd.Series(0.0, index=df.index)),
        'weight_kg': df['weight_kg'],
        'age_num': df['age_num'],
        'historical_podium_rate': df['historical_podium_rate'],
        'draw_numeric': df['draw_numeric']
    }
    feat_df = pd.DataFrame(features)
    # fill na
    feat_df = feat_df.fillna(0.0)
    # attach original metadata for display
