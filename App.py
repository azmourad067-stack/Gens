...
# -*- coding: utf-8 -*-
"""
Analyseur Hippique — Scraper Geny + Auto-training Hybride (DL + XGBoost)
Usage:
    streamlit run analyseur_hippique_geny.py
Notes:
    - Installer lxml (pip install lxml) pour parser correctement les pages Geny.
    - Si TensorFlow n'est pas installé, le script fonctionne avec XGBoost seul.
"""
...
def scrape_geny_stats(url, timeout=12):
    """
    Scrape a Geny stats page (or similar) and return a DataFrame
    with columns: Nom, Numéro de corde, Cote, Poids, Musique, Âge/Sexe, Jockey, Entraîneur, Gains
    """
    headers = {"User-Agent":"Mozilla/5.0 (Windows NT 10.0; Win64; x64)"}
    r = requests.get(url, headers=headers, timeout=timeout)
    # Ensure encoding (Geny often ISO-8859-1)
    if r.encoding is None or "utf" not in (r.encoding or "").lower():
        r.encoding = "ISO-8859-1"
    # Use lxml if available (faster / more robust)
    try:
        soup = BeautifulSoup(r.text, "lxml")
    except Exception:
        soup = BeautifulSoup(r.text, "html.parser")

    # Heuristics to find table of partants
    candidate_tables = soup.find_all("table")
    table = None
    for t in candidate_tables:
        # sample first 20 cells and check for hints
    ...
    if table is not None and len(table.find_all("tr")) > 0:
        for tr in table.find_all("tr"):
            ...
            entraineur = ""
            gains = 0
            # common pattern: first column number, second name
            if len(texts) >= 2:
                if re.match(r"^\d+$", texts[0].strip()):
                    num = texts[0].strip()
                    name = texts[1].strip()
                else:
                    # fallback: find first token with letters
                    for t in texts:
                        if re.search(r"[A-Za-zÀ-ÿ]", t):
                            name = t.strip()
                            break
            # detect musique-like tokens
            for t in texts[::-1]:
                if re.search(r"\d+[aA]|Da|Dm|mDa|[0-9]+a", t.replace(" ", "")):
                    musique = t.strip()
                    break
            # detect cote (numeric with comma/dot) from end to start
            for t in texts[::-1]:
            ...
            for t in texts[::-1]:
                ...
                if digits and len(digits) > 3:
                    try:
                        ...
                        pass
            # poids and age detection
            for t in texts:
                if re.search(r"\d{2}[,\.]\d|\d{2}\s?kg|\d{2}$", t):
                    poids = t.strip()
                    break
        ...
        # fallback: parse text blocks (Ecarts et statistiques)
        text = soup.get_text("\n", strip=True)
    ...
    # normalize names and fill defaults
...
if df_race is not None and len(df_race)>0:
    ...
    # prepare historique X/y if exists
